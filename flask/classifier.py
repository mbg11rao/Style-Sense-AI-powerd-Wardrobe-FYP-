...
print("before loading trained models")
    
sub_model = tf.keras.models.load_model('/home/uzair/Documents/FYP/Style Sense/backend/model_sub')
top_model = tf.keras.models.load_model('/home/uzair/Documents/FYP/Style Sense/backend/model_top')
bottom_model = tf.keras.models.load_model('/home/uzair/Documents/FYP/Style Sense/backend/model_bottom')
foot_model = tf.keras.models.load_model('/home/uzair/Documents/FYP/Style Sense/backend/model_shoes')

print("after loading trained models")
...
def single_classification(single_path):
    ...
    res_str = f"{res[0]}, {res[1]}, {color_classification(single_path)}, {res[3]}, {res[4]}" 
...
def map_detailed_color_to_group(color_name):
    """
    Maps detailed color names to generalized color groups.
    """
    base_colors = {
        'red': 'Red', 'orange': 'Orange', 'yellow': 'Yellow',
        'green': 'Green', 'blue': 'Blue', 'indigo': 'Indigo', 'violet': 'Violet',
        'black': 'Black', 'white': 'White', 'grey': 'Grey', 'pink': 'Pink', 'brown': 'Brown',
        'cyan': 'Cyan', 'magenta': 'Magenta', 'cream': 'Cream', 'teal': 'Teal',
        'lavender': 'Lavender', 'maroon': 'Maroon', 'navy': 'Navy', 'olive': 'Olive',
        'peach': 'Peach', 'gold': 'Gold', 'silver': 'Silver'
    }
    
    for key in base_colors:
        if key in color_name.lower():
            return base_colors[key]
    return 'Multi'  # Default to 'Multi' if no match found

def find_combo_by_top(top_color_name):
    """
    Recommend outfit color combinations based on detailed color names,
    utilizing advanced color theory and dynamic mapping.
    """
    color_groups = [
        'Red', 'Orange', 'Yellow', 'Green', 'Blue', 'Indigo', 'Violet',
        'Black', 'White', 'Grey', 'Pink', 'Brown', 'Cyan', 'Magenta',
        'Cream', 'Teal', 'Lavender', 'Maroon', 'Navy', 'Olive', 'Peach',
        'Gold', 'Silver', 'Multi'
    ]

    top_color_group = map_detailed_color_to_group(top_color_name)

    # Handle special cases for 'Multi' and monochromatic colors differently
    if top_color_group in ['Black', 'White', 'Grey', 'Cream', 'Multi']:
        bottom_color_group = random.choice(color_groups[:-1])  # Excluding 'Multi'
        shoes_color_group = random.choice(color_groups[:-1])
    else:
        top_color_index = color_groups.index(top_color_group)
        color_wheel_length = len(color_groups) - 1  # Excluding 'Multi'
        shift = random.choice([1, -1]) * random.randint(1, 3)  # Randomly choose adjacent or opposite colors

        bottom_color_index = (top_color_index + shift) % color_wheel_length
        shoes_color_index = (top_color_index - shift) % color_wheel_length

        bottom_color_group = color_groups[bottom_color_index]
        shoes_color_group = color_groups[shoes_color_index]
