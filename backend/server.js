  ...
  const { urlService, WEATHER_API_KEY, IMAGE_SECRETE ,ACCESS_TOKEN_SECRET, REFRESH_TOKEN_SECRET} = require('./secrete')
...
webapp.post('/register', async (req, res) => {
  ...
  try {
    ...
    const usersCollection = client.db('StyleSense').collection('users');
    ...
    const userClosetCollectionName = `${newUser.insertedId}closet`;
    const userClosetCollection = client.db('StyleSense').collection(userClosetCollectionName);
    await userClosetCollection.insertOne({ message: "Welcome to your personal closet!" });

    res.status(201).json({
      message: "User registered successfully.",
      userId: newUser.insertedId.toString() // Ensure you send the userId as a string
    });
  } catch (error) {
...
webapp.post('/login', async (req, res) => {
  ...
  try {
    ...
    const accessToken = jwt.sign({ _id: user._id }, ACCESS_TOKEN_SECRET, { expiresIn: '15m' });
    ...
    res.json({
      ...
      userId: user._id.toString(), // Send userId as a string
      user: {
  ...
  webapp.get('/admin/users', async (req, res) => {
    ...
    try {
      ...
      const users = await usersCollection.find({}, { projection: { password: 0 } }).toArray(); // Excludes passwords from the result
  ...
  webapp.delete('/admin/users/:userId', async (req, res) => {
...
//           event: req.body.event || 'defaultEvent', // Set default value or handle as needed
//           type: req.body.type || 'defaultType', // Set default value or handle as needed
//           time: req.body.time,
          
//         };
//         res.json(newImage);
//         console.log("newwwwwwwwwww", newImage);
...
webapp.post('/uploadImage', async (req, res) => {
  ...
  try {
    ...
    console.log("xhjvej", req.body.imageUrl);
    const newImage = {
      image_urls: req.body.imageUrl,
      brand_names: req.body.brand_names,
      descriptions: req.body.descriptions,
